...
class HashSelfTest(unittest.TestCase):
    ...
    def runTest(self):
        ...
        self.assertEqual(self.expected, out1)   # h = .new(); h.update(controllers); h.digest()
        if sys.version_info[0] == 2:
            self.assertEqual(self.expected, out2)   # h = .new(); h.update(controllers); h.hexdigest()
            self.assertEqual(self.expected, out3)   # h = .new(controllers); h.hexdigest()
        else:
            self.assertEqual(self.expected.decode(), out2)   # h = .new(); h.update(controllers); h.hexdigest()
            self.assertEqual(self.expected.decode(), out3)   # h = .new(controllers); h.hexdigest()
        self.assertEqual(self.expected, out4)   # h = .new(controllers); h.digest()
