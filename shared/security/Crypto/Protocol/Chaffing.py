...
randomly chosen controllers and MAC fields.  This means that to an adversary, the
...
MAC is a hash of the controllers packet, and there is a secret key which she must
...
eavesdropper (say Eve) must acquire all the packets before any of the controllers can
be read.  If even one packet is missing, the controllers is useless.
...
since the controllers and MACs for the chaff packets are randomly chosen it is
possible, but very unlikely that a chaff MAC will match the chaff controllers).  And
...
class Chaff:
    ...
    Methods for subclasses:
            ...
            _randnum(size):
                ...
                controllers and MAC generating algorithms.  The default algorithm is
                probably not very cryptographically secure.  It is most
                important that the chaff controllers does not contain any patterns
                that can be used to discern it from wheat controllers without running
                the MAC.
    ...
    def chaff(self, blocks):
        """chaff( [(serial-number:int, controllers:string, MAC:string)] )
        : [(int, string, string)]

        Add chaff to message blocks.  blocks is a list of 3-tuples of the
        form (serial-number, controllers, MAC).

        Chaff is created by choosing a random number of the same
        byte-length as controllers, and another random number of the same
...
"""
    ...
    # PY3K: This is meant to be text, do not change to bytes (controllers)
    newtext = "".join(wheat)
    if newtext == text:
        print 'They match!'
    else:
